...
<body>
    ...
    -->
						<c:forEach items="${ viewdList }" var="viewdList"
							varStatus="status">
							<c:url var="meetDetailPage" value="/meet/maindetail">
								<c:param name="mtid" value="${viewdList.mtid}" />
							</c:url>
							<div class="col" style="cursor: pointer;" id="clickEvent"
              				...
              				<rect width="100%" height="100%" fill="#55595c" />
									...
									<div>
										<c:choose>
											<c:when test="${viewdList.PCNT >= viewdList.nm_ppl}">
										...
										<div class="post-inner">
											<div class="row align-items-center">
                     						 ...
                     						 <path
													...
													<span class="text-end" id="heart${status.begin}"> <em>${viewdList.LCNT}</em>
													</span>
												</div>
											</div>
											<div id="post-sidoAndTitle${status.begin}">[
												${viewdList.sido} ] &lt;${viewdList.title}&gt;</div>
											<div id="post-mtDate${status.begin}">
												<span class="map">모집일정: ${viewdList.mt_date}</span>
											</div>
										</div>
										<div class="text-lg-end text-center">
											<small class="text-muted"> 모집인원: <span
												id="currentApplicants${status.begin}">${viewdList.PCNT}</span>
												/ <span id="maxApplicants${status.begin}">${viewdList.nm_ppl}</span>
											</small>
										</div>
									</div>
									<!-- 카드 끝 -->
